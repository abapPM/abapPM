...
CLASS lcl_nodes_helper DEFINITION FINAL.
  PUBLIC SECTION.
    ...
    DATA mt_nodes TYPE zif_abappm_ajson_types=>ty_nodes_tt READ-ONLY.
...
CLASS ltcl_json_utils DEFINITION
  ...
  PRIVATE SECTION.
    ...
    METHODS json_diff FOR TESTING RAISING zcx_abappm_ajson_error.
    METHODS json_diff_types FOR TESTING RAISING zcx_abappm_ajson_error.
    METHODS json_diff_arrays FOR TESTING RAISING zcx_abappm_ajson_error.
    METHODS json_merge FOR TESTING RAISING zcx_abappm_ajson_error.
    METHODS json_sort FOR TESTING RAISING zcx_abappm_ajson_error.
    METHODS is_equal FOR TESTING RAISING zcx_abappm_ajson_error.
...
CLASS ltcl_json_utils IMPLEMENTATION.
  ...
  METHOD json_diff.
    ...
    DATA:
      lv_json       TYPE string,
      lo_util       TYPE REF TO zcl_abappm_ajson_utilities,
      lo_insert     TYPE REF TO zif_abappm_ajson,
      lo_delete     TYPE REF TO zif_abappm_ajson,
      lo_change     TYPE REF TO zif_abappm_ajson,
...
*      '        "row": 4,\n' && " array delete
  ...
  METHOD json_merge.
    ...
    DATA:
      ...
      lv_json_b    TYPE string,
      lo_util      TYPE REF TO zcl_abappm_ajson_utilities,
      lo_merge     TYPE REF TO zif_abappm_ajson,
  ...
  METHOD is_equal.
    ...
    cl_abap_unit_assert=>assert_equals(
      act = zcl_abappm_ajson_utilities=>new( )->is_equal(
        ii_json_a = zcl_abappm_ajson=>parse( '{"a":1,"b":2}' )
        ii_json_b = zcl_abappm_ajson=>parse( '{"a":1,"b":2}' ) )
      exp = abap_true ).

...
CLASS lcl_node_iterator DEFINITION FINAL.
  PUBLIC SECTION.
    INTERFACES /apmg/if_apm_ajson_iterator.
    METHODS constructor
      IMPORTING
        ii_json      TYPE REF TO /apmg/if_apm_ajson
        iv_path      TYPE string
        iv_node_type TYPE /apmg/if_apm_ajson_types=>ty_node_type
      RAISING
        /apmg/cx_apm_ajson_error.

  PRIVATE SECTION.
    DATA mi_json TYPE REF TO /apmg/if_apm_ajson.
    DATA mv_node_type TYPE /apmg/if_apm_ajson_types=>ty_node_type.
    DATA mv_base_path TYPE string.
    DATA mr_cursor TYPE REF TO /apmg/if_apm_ajson_types=>ty_node.
...
CLASS lcl_node_iterator IMPLEMENTATION.
  ...
  METHOD constructor.
    ...
    IF NOT ( iv_node_type = /apmg/if_apm_ajson_types=>node_type-array OR iv_node_type = /apmg/if_apm_ajson_types=>node_type-object ).
      /apmg/cx_apm_ajson_error=>raise( |Iterator can iterate arrays or objects only ("{ iv_node_type }" passed)| ).
    ENDIF.

    mv_base_path = /apmg/cl_apm_ajson=>normalize_path( iv_path ).
    ...
    ELSEIF mv_node_type = /apmg/if_apm_ajson_types=>node_type-array AND lv_node_type <> mv_node_type.
      /apmg/cx_apm_ajson_error=>raise( |Array expected at: { iv_path }| ).
    ELSEIF mv_node_type = /apmg/if_apm_ajson_types=>node_type-object AND lv_node_type <> mv_node_type.
      /apmg/cx_apm_ajson_error=>raise( |Object expected at: { iv_path }| ).
    ENDIF.
  ...
  METHOD /apmg/if_apm_ajson_iterator~has_next.
    rv_yes = mv_has_next.
  ENDMETHOD.

  METHOD /apmg/if_apm_ajson_iterator~next.

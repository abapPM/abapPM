CLASS zcl_abappm_gui_css_processor DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC.
  ...
  PUBLIC SECTION.
    ...
    METHODS constructor
      IMPORTING
        !ii_asset_manager TYPE REF TO zif_abappm_gui_asset_manager.
  ...
  PRIVATE SECTION.
    ...
    DATA:
      mi_asset_manager TYPE REF TO zif_abappm_gui_asset_manager,
...
CLASS zcl_abappm_gui_css_processor IMPLEMENTATION.
  ...
  METHOD resolve_var_recursively.
    ...
    IF sy-subrc = 0.
      ...
      ENDDO.
    ELSE.
      zcx_abappm_error=>raise( |CSS variable { iv_variable_name } not resolvable| ).
    ENDIF.
  ENDMETHOD.
ENDCLASS.

CLASS zcl_abappm_gui_html_processor DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC.
  ...
  PUBLIC SECTION.
    ...
    INTERFACES zif_abappm_gui_html_processor.

    CONSTANTS c_css_build_name TYPE string VALUE 'css/bundle.css' ##NO_TEXT.
    CONSTANTS c_preprocess_marker TYPE string VALUE `<!-- abappm HTML preprocessor -->` ##NO_TEXT.
    CONSTANTS c_comment_start TYPE string VALUE `<!--` ##NO_TEXT.
    CONSTANTS c_comment_end TYPE string VALUE `-->` ##NO_TEXT.

    METHODS constructor
      IMPORTING
        !ii_asset_man TYPE REF TO zif_abappm_gui_asset_manager.
  ...
  METHOD FIND_HEAD_OFFSET.
    ...
    IF rv_head_end <= 0.
      rv_head_end = find( val = iv_html
                          ...
                          case = abap_false ).
      IF rv_head_end <= 0.
        zcx_abappm_error=>raise( 'HTML preprocessor: </head> not found' ).
      ENDIF.
    ENDIF.

CLASS zcl_abappm_gui_asset_manager DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC.
  ...
  PUBLIC SECTION.
    ...
    INTERFACES zif_abappm_gui_asset_manager.

    CLASS-METHODS create
      RETURNING
        VALUE(ri_asset_manager) TYPE REF TO zif_abappm_gui_asset_manager.
  ...
  PRIVATE SECTION.
    ...
    TYPES:
      BEGIN OF ty_asset_entry.
        INCLUDE TYPE zif_abappm_gui_asset_manager~ty_web_asset.
    ...
    METHODS load_asset
      ...
      RETURNING
        VALUE(rs_asset) TYPE zif_abappm_gui_asset_manager~ty_web_asset
      RAISING
        zcx_abappm_error.
...
CLASS zcl_abappm_gui_asset_manager IMPLEMENTATION.


  METHOD create.
    CREATE OBJECT ri_asset_manager TYPE zcl_abappm_gui_asset_manager.
  ENDMETHOD.
  ...
  METHOD load_asset.
    ...
    IF rs_asset-content IS INITIAL.
      zcx_abappm_error=>raise( |failed to load GUI asset: { is_asset_entry-url }| ).
    ENDIF.

  ENDMETHOD.


  METHOD zif_abappm_gui_asset_manager~get_all_assets.
  ...
  METHOD zif_abappm_gui_asset_manager~get_asset.

    FIELD-SYMBOLS <ls_a> LIKE LINE OF mt_asset_register.

    READ TABLE mt_asset_register WITH KEY url = iv_url ASSIGNING <ls_a>.
    IF <ls_a> IS NOT ASSIGNED.
      zcx_abappm_error=>raise( |Cannot find GUI asset: { iv_url }| ).
    ENDIF.
  ...
  METHOD zif_abappm_gui_asset_manager~get_text_asset.

    DATA ls_asset TYPE zif_abappm_gui_asset_manager~ty_web_asset.

    ls_asset = zif_abappm_gui_asset_manager~get_asset( iv_url ).

    IF ls_asset-type <> 'text'.
      zcx_abappm_error=>raise( |Not a text asset: { iv_url }| ).
    ENDIF.
  ...
  METHOD zif_abappm_gui_asset_manager~register_asset.

CLASS zcl_abappm_gui DEFINITION
  PUBLIC
  CREATE PUBLIC.

  PUBLIC SECTION.

    INTERFACES zif_abappm_gui_services.
    ...
    METHODS on_event
      FOR EVENT sapevent OF zif_abappm_html_viewer
      IMPORTING
        !action
        !frame
        !getdata
    ...
    METHODS constructor
      IMPORTING
        ...
        !ii_asset_man         TYPE REF TO zif_abappm_gui_asset_manager OPTIONAL
        !ii_hotkey_ctl        TYPE REF TO zif_abappm_gui_hotkey_ctl OPTIONAL
        !ii_html_processor    TYPE REF TO zif_abappm_gui_html_processor OPTIONAL
  ...
  PRIVATE SECTION.
    ...
    METHODS call_page
      IMPORTING
        !ii_page          TYPE REF TO zif_abappm_gui_renderable
    ...
    METHODS handle_action
      IMPORTING
        ...
        !it_postdata TYPE zif_abappm_html_viewer=>ty_post_data OPTIONAL.

    METHODS handle_error
      IMPORTING
        !ix_exception TYPE REF TO zcx_abappm_error.
...
CLASS zcl_abappm_gui IMPLEMENTATION.
  ...
  METHOD back_graceful.
    ...
    DATA li_handler TYPE REF TO zif_abappm_gui_event_handler.
    DATA ls_handled TYPE zif_abappm_gui_event_handler=>ty_handling_result.
    ...
    IF sy-subrc = 0.
      ls_handled = li_handler->on_event( zcl_abappm_gui_event=>new(
  ...
  METHOD cache_html.
    ...
    rv_url = zif_abappm_gui_services~cache_asset(
      iv_text    = iv_text
      iv_type    = 'text'
      iv_subtype = 'html' ).
  ...
  METHOD constructor.
    ...
    IF io_component IS BOUND.
      IF zcl_abappm_gui_utils=>is_renderable( io_component ) = abap_true.
        mi_cur_page ?= io_component. " direct page
      ELSE.
        IF zcl_abappm_gui_utils=>is_event_handler( io_component ) = abap_false.
          zcx_abappm_error=>raise( 'Component must be renderable or be an event handler' ).
        ENDIF.
  ...
  METHOD handle_action.
    ...
    DATA:
      lx_exception TYPE REF TO zcx_abappm_error,
      li_handler   TYPE REF TO zif_abappm_gui_event_handler,
      li_event     TYPE REF TO zif_abappm_gui_event,
      ls_handled   TYPE zif_abappm_gui_event_handler=>ty_handling_result.

    CREATE OBJECT li_event TYPE zcl_abappm_gui_event
      EXPORTING
        ii_gui_services = me
        iv_action       = iv_action
    ...
    TRY.
        ...
        CASE ls_handled-state.
          ...
          WHEN OTHERS.
            zcx_abappm_error=>raise( |Unknown action: { iv_action }| ).
        ENDCASE.

      CATCH zcx_abappm_cancel ##NO_HANDLER.
        " Do nothing = c_event_state-no_more_act
      CATCH zcx_abappm_error INTO lx_exception.
  ...
  METHOD handle_error.
    ...
    DATA: li_gui_error_handler TYPE REF TO zif_abappm_gui_error_handler,
...
*          IF mi_common_log->get_log_level( ) >= zif_abappm_log=>c_log_level-warning
*            zcl_abappm_log_viewer=>show_log( mi_common_log )
*          ENDIF
        ELSE.
          MESSAGE ix_exception TYPE 'S' DISPLAY LIKE 'E'.
        ENDIF.

      CATCH zcx_abappm_error cx_sy_move_cast_error INTO lx_exception.
  ...
  METHOD is_page_modal.
    ...
    DATA li_modal TYPE REF TO zif_abappm_gui_modal.
  ...
  METHOD render.
    ...
    DATA: lv_url  TYPE string,
          lv_html TYPE string,
          li_html TYPE REF TO zif_abappm_html.

    IF mi_cur_page IS NOT BOUND.
      zcx_abappm_error=>raise( 'GUI error: no current page' ).
    ENDIF.
  ...
  METHOD startup.
    ...
    DATA: lt_events TYPE cntl_simple_events,
          ...
          lt_assets TYPE zif_abappm_gui_asset_manager=>ty_web_assets.

    FIELD-SYMBOLS <ls_asset> LIKE LINE OF lt_assets.


    mi_html_viewer = zcl_abappm_gui_factory=>get_html_viewer( ).
    ...
    IF mi_asset_man IS BOUND.
      ...
      LOOP AT lt_assets ASSIGNING <ls_asset> WHERE is_cacheable = abap_true.
        zif_abappm_gui_services~cache_asset(
  ...
  METHOD zif_abappm_gui_services~cache_asset.
  ...
  METHOD zif_abappm_gui_services~get_current_page_name.

    DATA li_page_hoc TYPE REF TO zcl_abappm_gui_page_hoc.
    ...
    IF mi_cur_page IS BOUND.
      ...
      IF rv_page_name = 'ZCL_abappm_GUI_PAGE_HOC'.
  ...
  METHOD zif_abappm_gui_services~get_hotkeys_ctl.
    ri_hotkey_ctl = mi_hotkey_ctl.
  ENDMETHOD.


  METHOD zif_abappm_gui_services~get_html_parts.
    ro_parts = mo_html_parts.
  ENDMETHOD.


  METHOD zif_abappm_gui_services~get_log.
  ...
  METHOD zif_abappm_gui_services~register_event_handler.
    ASSERT ii_event_handler IS BOUND.
    INSERT ii_event_handler INTO mt_event_handlers INDEX 1.
  ENDMETHOD.


  METHOD zif_abappm_gui_services~register_page_asset.

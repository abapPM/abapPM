CLASS ltcl_event DEFINITION
  ...
  PRIVATE SECTION.
    ...
    METHODS query_wrong_data FOR TESTING RAISING zcx_abappm_error.
    METHODS form_wrong_data FOR TESTING RAISING zcx_abappm_error.
    METHODS query FOR TESTING RAISING zcx_abappm_error.
    METHODS form_data FOR TESTING RAISING zcx_abappm_error.
    METHODS immutability FOR TESTING RAISING zcx_abappm_error.
...
CLASS ltcl_event IMPLEMENTATION.
  ...
  METHOD query_wrong_data.
    ...
    DATA lo_map TYPE REF TO zcl_abappm_string_map.
  ...
  METHOD form_data.
    ...
    DATA li_cut TYPE REF TO zif_abappm_gui_event.
    DATA lo_map TYPE REF TO zcl_abappm_string_map.
    DATA lt_postdata TYPE zif_abappm_html_viewer=>ty_post_data.
...
CLASS zcl_abappm_gui_event DEFINITION LOCAL FRIENDS ltcl_html_action_utils.
...
CLASS ltcl_html_action_utils IMPLEMENTATION.
  ...
  METHOD parse_fields_unescape_nbsp.
    ...
    _given_string_is( '/src/ztest_rfc.fugr.xml=%3F&/src/ztest_rfc'
                   && zcl_abappm_gui_event=>gv_non_breaking_space
                   && zcl_abappm_gui_event=>gv_non_breaking_space
                   && zcl_abappm_gui_event=>gv_non_breaking_space
                   && 'rf.sush.xml=A' ).
  ...
  METHOD _when_fields_are_parsed_upper.
    ...
    mt_parsed_fields = zcl_abappm_gui_event=>parse_fields_upper_case_name( mv_given_parse_string ).

  ENDMETHOD.

  METHOD _when_fields_are_parsed.

    mt_parsed_fields = zcl_abappm_gui_event=>parse_fields( mv_given_parse_string ).
  ...
  METHOD parse_post_form_data.
    ...
    mt_parsed_fields = zcl_abappm_gui_event=>parse_post_form_data( lt_post_data ).
    _then_field_count_should_be( 2 ).
    _then_fields_should_be(
      iv_index = 1
      iv_name  = 'a'
    ...
    mt_parsed_fields = zcl_abappm_gui_event=>parse_post_form_data(

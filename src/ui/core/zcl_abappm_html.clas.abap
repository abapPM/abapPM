CLASS zcl_abappm_html DEFINITION
  PUBLIC
  CREATE PUBLIC.

  PUBLIC SECTION.

    INTERFACES zif_abappm_html.
    ...
    CLASS-METHODS create
      ...
      RETURNING
        VALUE(ri_instance) TYPE REF TO zif_abappm_html.
    ...
    CLASS-METHODS parse_data_attr
      ...
      RETURNING
        VALUE(rs_data_attr) TYPE zif_abappm_html=>ty_data_attr.
...
CLASS zcl_abappm_html IMPLEMENTATION.
  ...
  METHOD create.
    CREATE OBJECT ri_instance TYPE zcl_abappm_html.
...
* if everything is closings, there are no single tags
  ...
  METHOD zif_abappm_html~a.
    ...
    IF iv_opt CA zif_abappm_html=>c_html_opt-strong.
      lv_class = lv_class && ' emphasis'.
    ENDIF.
    IF iv_opt CA zif_abappm_html=>c_html_opt-cancel.
      lv_class = lv_class && ' attention'.
    ENDIF.
    IF iv_opt CA zif_abappm_html=>c_html_opt-crossout.
    ...
    lv_act  = iv_act.
    IF ( iv_act IS NOT INITIAL OR iv_typ = zif_abappm_html=>c_action_type-dummy )
        AND iv_opt NA zif_abappm_html=>c_html_opt-crossout.
      CASE iv_typ.
        WHEN zif_abappm_html=>c_action_type-url.
          IF iv_query IS NOT INITIAL.
            lv_act = lv_act && `?` && iv_query.
          ENDIF.
          lv_href  = | href="{ lv_act }"|.
        WHEN zif_abappm_html=>c_action_type-sapevent.
          IF iv_query IS NOT INITIAL.
            lv_act = lv_act && `?` && iv_query.
          ENDIF.
          lv_href  = | href="sapevent:{ lv_act }"|.
        WHEN zif_abappm_html=>c_action_type-onclick.
          lv_href  = ' href="#"'.
          lv_click = | onclick="{ iv_act }"|.
        WHEN zif_abappm_html=>c_action_type-dummy.
          lv_href  = ' href="#"'.
      ENDCASE.
    ENDIF.
  ...
  METHOD zif_abappm_html~add.

    DATA: lv_type       TYPE c,
          li_renderable TYPE REF TO zif_abappm_gui_renderable,
          lx_error      TYPE REF TO zcx_abappm_error,
          lo_html       TYPE REF TO zcl_abappm_html.
  ...
  METHOD zif_abappm_html~add_a.

    zif_abappm_html~add( zif_abappm_html~a(
      iv_txt   = iv_txt
      iv_act   = iv_act
      iv_query = iv_query
      iv_typ   = iv_typ
  ...
  METHOD zif_abappm_html~add_checkbox.

    zif_abappm_html~add( checkbox(
      iv_id      = iv_id
  ...
  METHOD zif_abappm_html~add_icon.

    zif_abappm_html~add( icon(
      iv_name    = iv_name
      iv_class   = iv_class
      iv_hint    = iv_hint
  ...
  METHOD zif_abappm_html~div.
    zif_abappm_html~wrap(
      iv_tag   = 'div'
  ...
  METHOD zif_abappm_html~is_empty.
    rv_yes = boolc( lines( mt_buffer ) = 0 ).
  ENDMETHOD.


  METHOD zif_abappm_html~render.
  ...
  METHOD zif_abappm_html~set_title.
    zif_abappm_html~mv_chunk_title = iv_title.
    ri_self = me.
  ENDMETHOD.


  METHOD zif_abappm_html~td.
    zif_abappm_html~wrap(
  ...
  METHOD zif_abappm_html~wrap.
    ...
    IF iv_format_single_line = abap_true AND iv_content IS NOT INITIAL.
      zif_abappm_html~add( lv_open_tag && iv_content && lv_close_tag ).
    ELSE.
      zif_abappm_html~add( lv_open_tag ).
      IF ii_content IS BOUND.
        zif_abappm_html~add( ii_content ).
      ELSEIF iv_content IS NOT INITIAL.
        zif_abappm_html~add( iv_content ).
      ENDIF.
      IF lv_close_tag IS NOT INITIAL.
        zif_abappm_html~add( lv_close_tag ).
      ENDIF.
    ENDIF.

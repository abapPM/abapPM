CLASS zcl_abappm_gui_event DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC.
  ...
  PUBLIC SECTION.
    ...
    INTERFACES zif_abappm_gui_event.

    CLASS-METHODS class_constructor.

    CLASS-METHODS new
      IMPORTING
        !ii_gui_services   TYPE REF TO zif_abappm_gui_services OPTIONAL
        !iv_action         TYPE clike
        !iv_getdata        TYPE clike OPTIONAL
        !it_postdata       TYPE zif_abappm_html_viewer=>ty_post_data OPTIONAL
      RETURNING
        VALUE(ro_instance) TYPE REF TO zcl_abappm_gui_event.
  ...
  PRIVATE SECTION.
    ...
    DATA mo_query TYPE REF TO zcl_abappm_string_map.
    ...
    CLASS-METHODS parse_post_form_data
      IMPORTING
        !it_post_data    TYPE zif_abappm_html_viewer=>ty_post_data
    ...
    CLASS-METHODS translate_postdata
      IMPORTING
        !it_postdata     TYPE zif_abappm_html_viewer=>ty_post_data
      RETURNING
...
CLASS zcl_abappm_gui_event IMPLEMENTATION.
  ...
  METHOD constructor.
    ...
    zif_abappm_gui_event~mi_gui_services = ii_gui_services.
    zif_abappm_gui_event~mv_action       = to_lower( iv_action ).
    zif_abappm_gui_event~mv_getdata      = iv_getdata.
    zif_abappm_gui_event~mt_postdata     = it_postdata.

    IF ii_gui_services IS BOUND.
      zif_abappm_gui_event~mv_current_page_name = ii_gui_services->get_current_page_name( ).
    ENDIF.
  ...
  METHOD translate_postdata.
    ...
    DATA: lt_post_data       TYPE zif_abappm_html_viewer=>ty_post_data,
...
* todo, more to be added here
  ...
  METHOD zif_abappm_gui_event~form_data.

    IF mo_form_data IS NOT BOUND.
      mo_form_data = fields_to_map( parse_post_form_data( zif_abappm_gui_event~mt_postdata ) ).
      mo_form_data->freeze( ).
    ENDIF.
  ...
  METHOD zif_abappm_gui_event~query.

    IF mo_query IS NOT BOUND.
      mo_query = fields_to_map( parse_fields( zif_abappm_gui_event~mv_getdata ) ).
      mo_query->freeze( ).
    ENDIF.

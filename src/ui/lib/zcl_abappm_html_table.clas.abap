CLASS zcl_abappm_html_table DEFINITION
  PUBLIC
  ...
  PUBLIC SECTION.
    ...
    CLASS-METHODS create
      IMPORTING
        !ii_renderer              TYPE REF TO zif_abappm_html_table OPTIONAL " Can be passed to renderer
        !is_initial_sorting_state TYPE zif_abappm_html_table=>ty_sorting_state OPTIONAL
          PREFERRED PARAMETER ii_renderer
      RETURNING
        VALUE(ro_instance)        TYPE REF TO zcl_abappm_html_table.
    ...
    METHODS define_column
      ...
      RETURNING
        VALUE(ro_self)   TYPE REF TO zcl_abappm_html_table.
    ...
    METHODS render
      IMPORTING
        !ii_renderer      TYPE REF TO zif_abappm_html_table OPTIONAL
        !it_data          TYPE ANY TABLE
        !iv_id            TYPE csequence OPTIONAL
        !iv_css_class     TYPE csequence OPTIONAL
        !iv_with_cids     TYPE abap_bool DEFAULT abap_false
        !is_sorting_state TYPE zif_abappm_html_table=>ty_sorting_state OPTIONAL
    ...
    CLASS-METHODS detect_sorting_request
      ...
      RETURNING
        VALUE(rs_sorting_request) TYPE zif_abappm_html_table=>ty_sorting_state.
  ...
  PRIVATE SECTION.
    ...
    DATA mi_renderer TYPE REF TO zif_abappm_html_table.
    DATA mt_columns TYPE ty_columns.
    DATA mi_html TYPE REF TO zif_abappm_html.
    DATA mv_with_cids TYPE abap_bool.
    DATA mv_table_id TYPE string.
    DATA ms_sorting_state TYPE zif_abappm_html_table=>ty_sorting_state.
...
CLASS zcl_abappm_html_table IMPLEMENTATION.
  ...
  METHOD define_column_group.
    ...
    IF lines( mt_columns ) > 0 AND mr_last_grp IS INITIAL.
      ...
      zcx_abappm_error=>raise( 'Start groups from the beginning' ).
    ENDIF.
  ...
  METHOD render.
    ...
    CREATE OBJECT mi_html TYPE zcl_abappm_html.
    ri_html = mi_html.
  ...
  METHOD render_row.
    ...
    DATA ls_render TYPE zif_abappm_html_table=>ty_cell_render.
    DATA lv_dummy TYPE string.
    DATA lt_attrs TYPE zif_abappm_html=>ty_data_attrs.
    ...
    LOOP AT mt_columns ASSIGNING <ls_col>.
      ...
      IF <ls_col>-from_field IS NOT INITIAL AND <ls_col>-from_field <> '-'.
        ...
        IF sy-subrc <> 0.
          zcx_abappm_error=>raise( |html_table: cannot assign field [{ <ls_col>-from_field }]| ).
        ENDIF.
  ...
  METHOD render_tbody.
    ...
    DATA ls_row_attrs TYPE zif_abappm_html_table=>ty_row_attrs.

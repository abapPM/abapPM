...
CLASS /apmg/cl_apm_html_form DEFINITION LOCAL FRIENDS ltcl_webgui_actions.
...
CLASS ltcl_webgui_actions IMPLEMENTATION.
  ...
  METHOD command_marker.
    DATA ls_command TYPE /apmg/if_apm_html_form=>ty_command.
    DATA lv_html TYPE string.

    ls_command-label = 'Save'.
    ls_command-action = 'save?key=1&name=example'.
    ls_command-cmd_type = /apmg/if_apm_html_form=>c_cmd_type-input_main.
  ...
  METHOD picker_marker.
    DATA ls_field TYPE /apmg/if_apm_html_form=>ty_field.
    DATA ls_attr TYPE /apmg/cl_apm_html_form=>ty_attr.
    DATA lv_html TYPE string.

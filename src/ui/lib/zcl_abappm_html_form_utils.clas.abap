CLASS zcl_abappm_html_form_utils DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC.
...
* adapted: html_form
  PUBLIC SECTION.
    ...
    CLASS-METHODS create
      IMPORTING
        !io_form            TYPE REF TO zcl_abappm_html_form
      RETURNING
        VALUE(ro_form_util) TYPE REF TO zcl_abappm_html_form_utils.
...
CLASS zcl_abappm_html_form_utils IMPLEMENTATION.
  ...
  METHOD exit.
    ...
    IF is_dirty(
      ...
      lv_answer = zcl_abappm_gui_factory=>get_popups( )->popup_to_confirm(
      ...
      IF lv_answer = '1'.
        rv_state = zcl_abappm_gui=>c_event_state-go_back_to_bookmark.
      ELSE.
        rv_state = zcl_abappm_gui=>c_event_state-no_more_act.
      ENDIF.
    ELSE.
      rv_state = zcl_abappm_gui=>c_event_state-go_back_to_bookmark.
    ENDIF.
  ...
  METHOD is_empty.
    ...
    DATA:
      lt_fields TYPE zif_abappm_html_form=>ty_fields,
    ...
    LOOP AT lt_fields ASSIGNING <ls_field> WHERE type <> zif_abappm_html_form=>c_field_type-field_group.
      lv_value = condense(
        val = io_form_data->get( <ls_field>-name )
        del = ` ` ).

      IF <ls_field>-type = zif_abappm_html_form=>c_field_type-number.
        rv_empty = boolc( lv_value IS INITIAL OR lv_value = '0' ).
      ELSEIF <ls_field>-type = zif_abappm_html_form=>c_field_type-table.
        lv_rows = io_form_data->get( |{ <ls_field>-name }-{ zif_abappm_html_form=>c_rows }| ).
        DO lv_rows TIMES.
          lv_row = sy-index.
          ...
          ENDDO.
        ENDDO.
      ELSEIF <ls_field>-type = zif_abappm_html_form=>c_field_type-textarea.
  ...
  METHOD normalize.
    ...
    lt_fields = mo_form->get_fields( ).
    LOOP AT lt_fields ASSIGNING <ls_field> WHERE type <> zif_abappm_html_form=>c_field_type-field_group
      AND type <> zif_abappm_html_form=>c_field_type-hidden.
      ...
      IF <ls_field>-type = zif_abappm_html_form=>c_field_type-checkbox.
        ro_form_data->set(
          iv_key = <ls_field>-name
          iv_val = boolc( lv_value = 'on' ) ) ##TYPE.
      ELSEIF ( <ls_field>-type = zif_abappm_html_form=>c_field_type-text
          OR <ls_field>-type = zif_abappm_html_form=>c_field_type-textarea )
          AND <ls_field>-upper_case = abap_true.
        ro_form_data->set(
          iv_key = <ls_field>-name
          iv_val = to_upper( lv_value ) ).
      ELSEIF <ls_field>-type = zif_abappm_html_form=>c_field_type-number.
      ...
      ELSEIF <ls_field>-type = zif_abappm_html_form=>c_field_type-table.
        ...
        ENDDO.
        ro_form_data->set(
          iv_key = |{ <ls_field>-name }-{ zif_abappm_html_form=>c_rows }|
          iv_val = |{ lv_rows }| ).
      ELSEIF <ls_field>-type = zif_abappm_html_form=>c_field_type-textarea.
  ...
  METHOD validate.
    ...
    LOOP AT lt_fields ASSIGNING <ls_field>.
      ...
      ENDIF.
      CASE <ls_field>-type.
        WHEN zif_abappm_html_form=>c_field_type-text.
          ...
          ELSE.
            ...
            ENDIF.
          ENDIF.
        WHEN zif_abappm_html_form=>c_field_type-number.
          TRY.
              lv_number = lv_value.
            CATCH cx_root.
              ro_validation_log->set(

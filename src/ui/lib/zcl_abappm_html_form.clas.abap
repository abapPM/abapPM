CLASS zcl_abappm_html_form DEFINITION
  PUBLIC
...
* adapted: gui_services
  PUBLIC SECTION.
    ...
    INTERFACES zif_abappm_html_form.
    INTERFACES zif_abappm_gui_hotkeys.
    ...
    METHODS command
      IMPORTING
        ...
        !iv_cmd_type   TYPE i DEFAULT zif_abappm_html_form=>c_cmd_type-input
      RETURNING
        VALUE(ro_self) TYPE REF TO zcl_abappm_html_form.
    ...
    METHODS get_fields
      RETURNING
        VALUE(rt_fields) TYPE zif_abappm_html_form=>ty_fields.
  ...
  PRIVATE SECTION.
    ...
    DATA:
      ...
      mt_fields    TYPE zif_abappm_html_form=>ty_fields,
      mt_commands  TYPE STANDARD TABLE OF zif_abappm_html_form=>ty_command,
      mv_form_id   TYPE string,
      mv_help_page TYPE string.

    METHODS render_field
      IMPORTING
        !ii_html           TYPE REF TO zif_abappm_html
        !io_values         TYPE REF TO zcl_abappm_string_map
        !io_validation_log TYPE REF TO zcl_abappm_string_map
        !is_field          TYPE zif_abappm_html_form=>ty_field
    ...
    METHODS render_command
      IMPORTING
        !ii_html TYPE REF TO zif_abappm_html
        !is_cmd  TYPE zif_abappm_html_form=>ty_command.
...
CLASS zcl_abappm_html_form IMPLEMENTATION.
  ...
  METHOD checkbox.
    ...
    ls_field-type     = zif_abappm_html_form=>c_field_type-checkbox.
  ...
  METHOD column.
    ...
    ASSERT sy-subrc = 0.
    ASSERT <ls_last>-type = zif_abappm_html_form=>c_field_type-table.
  ...
  METHOD create.
    ...
    ro_form->mv_webgui = zcl_abappm_gui_factory=>get_frontend_services( )->is_webgui( ).
  ...
  METHOD hidden.
    ...
    ls_field-type  = zif_abappm_html_form=>c_field_type-hidden.
    ls_field-name  = iv_name.
    APPEND ls_field TO mt_fields.
  ...
  METHOD number.
    ...
    ls_field-type     = zif_abappm_html_form=>c_field_type-number.
    ls_field-name     = iv_name.
    ls_field-label    = iv_label.
    ls_field-readonly = iv_readonly.
    ls_field-min      = iv_min.
  ...
  METHOD option.
    ...
    READ TABLE mt_fields INDEX lv_size ASSIGNING <ls_last>.
    ASSERT sy-subrc = 0.
    ASSERT <ls_last>-type = zif_abappm_html_form=>c_field_type-radio. " Or dropdown - TODO in future
  ...
  METHOD radio.
    ...
    ls_field-type  = zif_abappm_html_form=>c_field_type-radio.
    ls_field-name  = iv_name.
    ls_field-label = iv_label.
  ...
  METHOD render.
    ...
    ENDIF.
    LOOP AT mt_commands ASSIGNING <ls_cmd> WHERE cmd_type = zif_abappm_html_form=>c_cmd_type-input_main.
    ...
    LOOP AT mt_fields ASSIGNING <ls_field>.
      AT FIRST.
        IF <ls_field>-type <> zif_abappm_html_form=>c_field_type-field_group.
          ri_html->add( |<ul>| ).
        ENDIF.
      ENDAT.
  ...
  METHOD render_command_link.
    ...
    IF is_cmd-cmd_type = zif_abappm_html_form=>c_cmd_type-input_main.
  ...
  METHOD render_field.
    ...
    IF is_field-type <> zif_abappm_html_form=>c_field_type-textarea.
    ...
    ENDIF.
    IF is_field-type = zif_abappm_html_form=>c_field_type-text AND is_field-max BETWEEN 1 AND 20.
      " Reduced width for short fields
      lv_item_class = lv_item_class && ' w40'.
    ENDIF.
    IF is_field-type = zif_abappm_html_form=>c_field_type-hidden.
    ...
    CASE is_field-type.
      WHEN zif_abappm_html_form=>c_field_type-text OR zif_abappm_html_form=>c_field_type-number.
      ...
      WHEN zif_abappm_html_form=>c_field_type-textarea.
      ...
      WHEN zif_abappm_html_form=>c_field_type-checkbox.
      ...
      WHEN zif_abappm_html_form=>c_field_type-radio.
      ...
      WHEN zif_abappm_html_form=>c_field_type-table.
      ...
      WHEN zif_abappm_html_form=>c_field_type-hidden.
  ...
  METHOD render_field_table.
    ...
    lv_rows = io_values->get( |{ is_field-name }-{ zif_abappm_html_form=>c_rows }| ).
    ...
    lv_value = |{ is_field-name }-{ zif_abappm_html_form=>c_rows }|.
  ...
  METHOD render_field_text.
    ...
    IF is_field-type = zif_abappm_html_form=>c_field_type-number.
      lv_type = 'number'.
  ...
  METHOD start_group.
    ...
    ls_field-type  = zif_abappm_html_form=>c_field_type-field_group.
    ls_field-label = iv_label.
    ls_field-name  = iv_name.
    ls_field-hint  = iv_hint.
  ...
  METHOD table.
    ...
    ls_field-type  = zif_abappm_html_form=>c_field_type-table.
    ls_field-name  = iv_name.
    ls_field-label = iv_label.
    ls_field-hint  = iv_hint.
  ...
  METHOD text.
    ...
    ls_field-type       = zif_abappm_html_form=>c_field_type-text.
    ls_field-name       = iv_name.
    ls_field-label      = iv_label.
  ...
  METHOD textarea.
    ...
    ls_field-type        = zif_abappm_html_form=>c_field_type-textarea.
    ls_field-name        = iv_name.
    ls_field-label       = iv_label.
    ls_field-readonly    = iv_readonly.
    ls_field-hint        = iv_hint.
  ...
  METHOD zif_abappm_gui_hotkeys~get_hotkey_actions.
    ...
    FIELD-SYMBOLS: <ls_command> TYPE zif_abappm_html_form=>ty_command.

    ls_hotkey_action-ui_component = |Form-{ mv_form_id }|.

    READ TABLE mt_commands WITH KEY cmd_type = zif_abappm_html_form=>c_cmd_type-input_main

CLASS zcl_abappm_html_toolbar DEFINITION
  PUBLIC
  CREATE PUBLIC.
  ...
  PUBLIC SECTION.
    ...
    CLASS-METHODS create
      ...
      RETURNING
        VALUE(ro_instance) TYPE REF TO zcl_abappm_html_toolbar.
    ...
    METHODS add
      IMPORTING
        !iv_txt        TYPE string
        !io_sub        TYPE REF TO zcl_abappm_html_toolbar OPTIONAL
        !iv_typ        TYPE c DEFAULT zif_abappm_html=>c_action_type-sapevent
      ...
      RETURNING
        VALUE(ro_self) TYPE REF TO zcl_abappm_html_toolbar.
    ...
    METHODS get_hotkeys
      ...
      RETURNING
        VALUE(rt_hotkeys)  TYPE zif_abappm_gui_hotkeys=>ty_hotkeys_with_descr.
  ...
  PRIVATE SECTION.
    ...
    TYPES:
      BEGIN OF ty_item,
        ...
        act      TYPE string,
        ico      TYPE string,
        sub      TYPE REF TO zcl_abappm_html_toolbar,
        opt      TYPE c LENGTH 1,
        typ      TYPE c LENGTH 1,
        cur      TYPE abap_bool,
        chk      TYPE abap_bool,
...
CLASS zcl_abappm_html_toolbar IMPLEMENTATION.


  METHOD add.
    DATA ls_item TYPE ty_item.

    ASSERT iv_typ = zif_abappm_html=>c_action_type-separator  " sep doesn't have action
      OR iv_typ = zif_abappm_html=>c_action_type-onclick      " click may have no action (assigned in JS)
      OR iv_typ = zif_abappm_html=>c_action_type-dummy        " dummy may have no action
  ...
  METHOD render_items.
    ...
    LOOP AT mt_items ASSIGNING <ls_item>.
      ...
      IF <ls_item>-typ = zif_abappm_html=>c_action_type-separator.
      ...
      ELSE.
        ri_html->add_a( iv_txt   = lv_icon && lv_txt
                        iv_typ   = zif_abappm_html=>c_action_type-dummy
                        iv_act   = ''
                        iv_id    = <ls_item>-id
                        iv_opt   = <ls_item>-opt
                        iv_title = <ls_item>-title

CLASS zcl_abappm_exception_viewer DEFINITION
  PUBLIC
  CREATE PUBLIC.
  ...
  PRIVATE SECTION.
    DATA:
      mx_error     TYPE REF TO zcx_abappm_error,
...
CLASS zcl_abappm_exception_viewer IMPLEMENTATION.
  ...
  METHOD get_top_of_callstack.
    ...
    IF sy-subrc <> 0.
      zcx_abappm_error=>raise( |Callstack is empty| ).
    ENDIF.
  ...
  METHOD show_callstack.
    ...
    DATA ls_position TYPE zif_abappm_popups=>ty_popup_position.

...
CLASS lcl_bcp47_language_table IMPLEMENTATION.
  ...
  METHOD sap1_to_bcp47.
    ...
    IF re_bcp47 IS INITIAL.
      zcx_abappm_error=>raise( |Could not map SAP1 language code { im_sap1 } to BCP47 language code.| ).
    ENDIF.
  ENDMETHOD.
  ...
  METHOD bcp47_to_sap1.
    ...
    IF re_sap1 IS INITIAL.
      zcx_abappm_error=>raise( |Could not map BCP47 language code { im_bcp47 } to SAP1 language code.| ).
    ENDIF.
  ENDMETHOD.
  ...
  METHOD sap1_to_sap2.
    ...
    IF re_sap2 IS INITIAL.
      zcx_abappm_error=>raise( |Could not map SAP1 language code { im_sap1 } to SAP2 language code.| ).
    ENDIF.
  ENDMETHOD.
  ...
  METHOD sap2_to_sap1.
    ...
    IF re_sap1 IS INITIAL.
      zcx_abappm_error=>raise( |Could not map SAP2 language code { im_sap2 } to SAP1 language code.| ).
    ENDIF.
  ENDMETHOD.
